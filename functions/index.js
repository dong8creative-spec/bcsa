...
// 카카오 로그인: 인증 코드로 액세스 토큰 교환 → 사용자 정보 조회 → Firebase 커스텀 토큰 발급 → 프론트 리다이렉트
...
app.get('/api/auth/kakao/callback', async (req, res) => {
  ...
  const baseUrl = process.env.FUNCTION_URL || `https://${req.get('host') || 'apibid-oytjv32jna-du.a.run.app'}`;
